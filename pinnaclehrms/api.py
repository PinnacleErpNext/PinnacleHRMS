...
import uuid
import json
from pinnaclehrms.utility.salary_calculator import createPaySlips, getEmpRecords, calculateMonthlySalary
...
def email_pay_slips(pay_slips=None, raw_data=None):
    ...
    for pay_slip_name in data:
        ...
        message = frappe.render_template("pinnaclehrms/public/templates/pay_slip.html", context)
...
def get_pay_slip_report(year=None, month=None, curr_user=None):
    ...
    if 'All' in user_roles or 'HR User' in user_roles or 'HR Manager' in user_roles:
        ...
        """, (curr_user, curr_user), as_dict=True)
        
        if not data:
            raise frappe.ValidationError("No Employee Data found or you don't have access!")
        
        employee_id = data[0].get('name')
        
        filter_condition = "WHERE ps.year = %s AND ps.month_num = %s AND ps.employee_id = %s"
        values = [year, month, employee_id]
    
    records = frappe.db.sql(query + " " + filter_condition, values)
    
    if not records:
        frappe.msgprint(msg='No records found for the specified year and month.', title='Warning!')
        return []
    ...
    for record in records:
        (
            pay_slip_name, year, month, employee_id, employee_name, company, designation, department, personal_email,
            standard_working_days, pan_number, date_of_joining, basic_salary, per_day_salary, 
            actual_working_days, absent, total, net_payble_amount, salary_particulars, salary_days, 
            salary_rate, salary_effective_percentage, salary_amount, earnings_type, earnings_amount
        ) = record
