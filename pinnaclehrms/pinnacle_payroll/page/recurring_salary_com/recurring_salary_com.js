...
`).appendTo(page.body);
		...
		const salaryComp = frappe.ui.form.make_control({
			df: {
				...
				change: function () {
					...
					if (compName) {
						frappe.db.get_value("Salary Component", compName, "type").then((r) => {
	...
	function generatePreview() {
		...
		const empId = employeeField.get_value();
		const salaryComp = $("#salary-component-rows .salary-component input").val();
		const totalAmount =
			parseFloat($("#salary-component-rows .total-amount input").val()?.replace(/,/g, "")) ||
			0;
		const numMonths = parseInt($("#salary-component-rows .months input").val()) || 0;
		...
		if (startDate.includes("-")) {
			...
			if (parts[0].length === 2) startDate = `${parts[2]}-${parts[1]}-${parts[0]}`;
		}

		frappe.call({
			method: "pinnaclehrms.pinnacle_payroll.page.recurring_salary_com.recurring_salary_com.get_existing_additional_salaries",
			args: {
				employee: empId,
			...
			callback: function (res) {
				...
				for (let i = 0; i < numMonths; i++) {
					...
					const status = existingMonths.includes(periodLabel) ? "Existing" : "New";
	...
	$("#save-components").on("click", function () {
		...
		if (!salaryComp || !totalAmount || !numMonths || !startDate) {
			frappe.msgprint("Please fill all Salary Component details before saving.");
			return;
		}
