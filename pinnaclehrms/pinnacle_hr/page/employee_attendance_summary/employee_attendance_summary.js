frappe.pages["employee-attendance-summary"].on_page_load = function (wrapper) {
  ...
  get_btn.$input.on("click", function () {
    ...
    frappe.call({
      method:
        "pinnaclehrms.pinnacle_hr.page.employee_attendance_summary.employee_attendance_summary.get_data",
          ...
          r.message.forEach((d) => {
            table += `
						<tr class="emp-row" id="emp-${d.employee}">
							...
							<td>
								<button
									class="btn btn-xs btn-default expand-row"
									data-emp="${d.employee}">
									▶
								</button>
							</td>
							...
							<td>${d.full_day || 0}</td>
							<td>${d.sunday_working || 0}</td>

							<td>${d.late_early || 0}</td>
							<td>${d.late_and_early || 0}</td>

							<td>${d.three_fourth_day || 0}</td>
							<td>${d.sixty_five_particular || 0}</td>

							<td>${d.half_day || 0}</td>
							<td>${d.forty_particular || 0}</td>

							<td>${d.quarter_day || 0}</td>
							<td>${d.fifteen_particular || 0}</td>
  ...
  download_btn.$input.on("click", function () {
    ...
    frappe.call({
      method:
        "pinnaclehrms.pinnacle_hr.page.employee_attendance_summary.employee_attendance_summary.download_excel",
  ...
  function attach_expand_events() {
    $(".expand-row").click(function () {
      ...
      frappe.call({
        method:
          "pinnaclehrms.pinnacle_hr.page.employee_attendance_summary.employee_attendance_summary.get_employee_month_breakdown",
            ...
            res.message.forEach((m) => {
              rows += `
							<tr
								...
								<td>${m.full_day || 0}</td>
								<td>${m.sunday_working || 0}</td>

								<td>${m.late_early || 0}</td>
								<td>${m.late_and_early || 0}</td>

								<td>${m.three_fourth_day || 0}</td>
								<td>${m.sixty_five_particular || 0}</td>

								<td>${m.half_day || 0}</td>
								<td>${m.forty_particular || 0}</td>

								<td>${m.quarter_day || 0}</td>
								<td>${m.fifteen_particular || 0}</td>
