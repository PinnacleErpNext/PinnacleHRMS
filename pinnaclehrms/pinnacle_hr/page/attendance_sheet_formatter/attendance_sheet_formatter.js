...
frappe.pages["attendance-sheet-formatter"].on_page_show = function (wrapper) {
	...
	function setupDownloadButton() {
		document.getElementById("download-template-btn").addEventListener("click", () => {
			const data = [
				["Employee", "Employee Name", "Attendance Date", "Shift", "In Time", "Out Time"],
			];
	...
	$("#load-raw-data-btn").on("click", async function () {
		...
		if (!payrollFrom || !payrollTo) return frappe.msgprint("Select Payroll Period.");
		...
		try {
			const res = await fetch(
				"/api/method/pinnaclehrms.pinnacle_hr.page.attendance_sheet_formatter.attendance_sheet_formatter.load_raw_attendance_data",
				{
					method: "POST",
			...
			console.log(data);
			if (!res.ok || !data.message) return frappe.msgprint("❌ Error loading data");
    ...
    <div class="scrollable-table-container">
        ...
        <tr>
            ...
            <td>
                <span class="badge bg-secondary">PUNCH</span>
            </td>
            <td>${row.time || ""}</td>
	...
	$("#generate-preview-btn").on("click", function () {
		...
		frappe.call({
			method: "pinnaclehrms.pinnacle_hr.page.attendance_sheet_formatter.attendance_sheet_formatter.preview_final_attendance_sheet",
			args: {
			...
			callback: function (r) {
				...
				if (!r.message || !Array.isArray(r.message.data)) {
					return frappe.msgprint("❌ Error generating preview");
				}

				// 🔹 Backend now returns FLAT LIST
				previewData = r.message.data;
				attendanceData = previewData;
            ...
            <table id="attendance-table" class="table table-bordered table-sm">
                ...
                <tr>
                    ...
                    <td>${row.shift || ""}</td>
                    <td>
                        <span class="badge ${row.log_type === "IN" ? "bg-success" : "bg-danger"}">
                            ${row.log_type}
                        </span>
                    </td>
                    <td>${row.time || ""}</td>
                    <td>${row.punch_from || ""}</td>
                </tr>`;
				});
	...
	$("#validate-btn").on("click", function () {
		if (!attendanceData.length) return frappe.msgprint("Generate preview first");

		frappe.dom.freeze("Validating...");

		frappe.call({
			method: "pinnaclehrms.pinnacle_hr.page.attendance_sheet_formatter.attendance_sheet_formatter.validate_attendance_data",
	...
	function renderValidatedTable() {
        ...
        <thead><tr><th>#</th><th>Employee</th><th>Name</th><th>Date</th><th>Shift</th><th>Log Type</th><th>Time</th><th>Punch From</th></tr></thead><tbody>`;
		...
		Object.keys(validatedRecord)
			...
			.forEach((emp) => {
                ...
                <td>${row.log_type}</td><td>${row.time}</td>
                <td>${row.punch_from}</td></tr>`;
				});
			});
	...
	function renderNonValidatedTable() {
		...
		let html = `<table class="table table-bordered table-danger table-sm">
        <thead><tr><th>#</th><th>Emp</th><th>Name</th><th>Date</th><th>Shift</th>
        <th>Time</th><th>Errors</th></tr></thead><tbody>`;
		...
		nonValidatedRecord.forEach((r, i) => {
            ...
            <td>${r.shift || ""}</td>
            <td><input type="time" value="${r.time || ""}" class="form-control in-time"/></td>
            <td class="text-danger small">${(r.errors || []).join(", ")}</td></tr>`;
		});

		html += `</tbody></table>
        <div class="text-center mt-2">
        <button id="revalidate-btn" class="btn btn-warning">Re-Validate</button>
        </div>`;
	...
	$(document).on("click", "#revalidate-btn", function () {
		...
		$("#non-validated-section tbody tr").each(function () {
			corrected.push({
				...
				in_time: $(this).find("td:eq(5) input").val(),
				out_time: $(this).find("td:eq(6) input").val(),
			});
		});
		...
		frappe.call({
			...
			callback: function (r) {
				...
				Object.keys(r.message.validated || {}).forEach((emp) => {
					...
					validatedRecord[emp] = validatedRecord[emp].concat(r.message.validated[emp]);
				});
	...
	$("#import-validated-btn").on("click", function () {
		frappe.call({
			method: "pinnaclehrms.pinnacle_hr.page.attendance_sheet_formatter.attendance_sheet_formatter.create_data_import_for_attendance",
	...
	async function downloadExcel(data, filename) {
		try {
			...
			const res = await fetch(
				"/api/method/pinnaclehrmsv.pinnacle_hr.page.attendance_sheet_formatter.attendance_sheet_formatter.download_final_attendance_excel",
				{
					method: "POST",
					headers: { "X-Frappe-CSRF-Token": frappe.csrf_token },
					body: formData,
			...
			if (contentType && contentType.includes("spreadsheetml")) {
				...
				frappe.show_alert({ message: `✅ ${filename} Downloaded`, indicator: "green" });
			} else {
